...
class ResponseDialogue(object):
    def __init__(self,agent,environment,responseDialogue_prompt_path=r'G:\毕业设计\pythonProject\agent_v1\agent_v_0_0_2\prompt\uniqueBehavior\responsedialogue.txt'):

        super(ResponseDialogue, self).__init__()

        self.agent = agent
        self.type = '具体行动结果'

        self.prompt_path = responseDialogue_prompt_path
    ...
    def extract_dialogue_info(self,dialogue_string):
        dialogue_initiator_pattern = r'回复对话的发起人：\[([^\]]+)\]'
        target_names_pattern = r'姓名：([^\]]+)'
        dialogue_content_pattern = r'回复对话的内容：\[([^\]]+)\]'
    ...
    def uniqueBehavior(self):
        ...
        ##begin特有行动编码区域##
        self.environment.UI = src + '对' + ';'.join(tar_list) + '回复了一次对话。' + '回复对话的内容是:' + '\n' + content
        print('**********BEGIN:环境描述**********')
        ...
        #注3:特有行动结果必须形成一次记忆记录。
        ##begin特有行动编码区域##
        desc = '我对' + ';'.join(tar_list) + '回复了一次对话。' + '回复对话的内容是:' + '\n' + content
